...
const ProductCard = (props) =>{
    ...
    const {name, price, imageId, description, category } = props?.data?.card?.info;
    return(
        <>
        <div className="flex items-center justify-center">
            <div className="w-full max-w-md bg-white shadow-lg rounded-lg flex m-2">
                ...
                <p className="text-lg text-green-500 font-semibold">Rs. {price/100}</p>
                </div>

                <div className="w-1/3 p-6 relative">
                    <img src={CDN_URL+imageId} alt="Product Image" className="object-contain h-20 w-48"/>
                    <button className="absolute bottom-2 right-2 bg-blue-500 text-white px-2 rounded hover:bg-blue-600">
                     Add +
                    </button>
                </div>
            </div>
